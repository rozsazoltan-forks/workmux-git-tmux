...
| `--name-window <name>`         | Override the tmux window target name for this command. In window mode, creates or selects `<window_prefix><name>` while keeping the worktree directory derived from the branch or `--name`. Cannot be used with session mode or multi-worktree generation.                               |
| `--name-session <name>`        | Override the tmux session target for this command. In session mode, creates or selects `<window_prefix><name>`. In window mode, creates the window inside the named tmux session without applying `window_prefix` to that parent session. Cannot be used with multi-worktree generation. |
...
5. Creates a new tmux window named `<window_prefix><handle>` (e.g., `wm-feature-auth` with `window_prefix: wm-`). With `--name-window`, the window target uses that name instead of `<handle>`. With `--mode session` or `--session`, the worktree is created in its own dedicated tmux session, and `--name-session` can override that session target.
...
workmux add feature/long-descriptive-branch-name --name-window review-short

# Create a custom-named window inside an existing PR review session
workmux add feature/review --name-session prs --name-window review-123
...
workmux add feature/review --mode session --name-session review-123
